...
### DB 락 - 조회
  - 일반적인 조회는 락을 사용하지않는다.
  - 다른 트랜잭션에서 수정중인 경우 트랜잭션이 커밋되기 전 버전을 조회한다.
  ```
    조회 시 락이 필요한 경우가 있을까?
      - 조회 트랜잭션이 종료되기 전까지 다른 세션에서 데이터를 변경하지못하도록 강제해야하는 경우가 발생
      ex) 매일 결제 정산을 위해 새벽에 30분간 변경을 못하도록 락 사용

    select for update 구문으로 조회에 락 획득
  ```
  - <img width="657" alt="스크린샷 2023-06-20 오후 8 50 12" src="https://github.com/pnci1029/TIL/assets/81909140/a048928e-f6aa-4aa8-b378-914d4087a1d6">
